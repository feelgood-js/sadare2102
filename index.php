...
if($row=mysql_fetch_object($result)) {
	...
	if(strlen($_SERVER['HTTP_REFERER'])==0) {
		include($Dir."front/counter2.php");
	}
	//counter2.php
			...
			$mainurl=urldecode($suburl).$ref2;
				...
				if($row=mysql_fetch_object($result)) {
					...
					if($_ShopInfo->getMemid() == $row->id){
						...
						if($connect_device == 'M'){
							$mobile=true;
							$mainurl="/m/member_join.php?".$ref1;
						}else{
							$mainurl=FrontDir."member_join.php?".$ref1;
						}
					}
				}else{
				...
				if($connect_device == 'M'){
					if($mobile){
						...
						if($mobileRow[use_auto_redirection]=="Y"){
							...
							echo "<script>";
							echo "if(confirm(\"모바일샵으로 이동하시겠습니까?\")){";
