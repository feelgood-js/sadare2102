    ...
    if(_empty($onload)) {
        ...
        if(!$onload) {
            ...
            if(!$onload) {
				...
				$sql = "INSERT yc_member SET ";
                if (_empty($loginType)) {
                    $sql.= "m_pass		= '".md5($passwd1)."', ";
                }
                $sql.= "m_nick	= '".$name."', ";
                $sql.= "m_email		= '".$email."', ";
                $sql.= "m_gender		= '".$gender."', ";
                $sql.= "m_birth		= '".(substr($birth, 0, 4))."'"; 
...
<script type="text/javascript">
 $(function(){
  var image = 'url("./img/iu2.png")'; //img URL
  var width = 34; //px
  var height = 34; //px
  var divClass = 'check'; //Div에 추가할 클래스명

  $('.ckbtn').each(function(){
   $(this).wrap("<div>").css({display:"none"});
   $(this).parent("div").addClass(divClass).css({
   ...
   if($(this).is(":checked")){
    $(this).parent("div").css({ backgroundPosition: "0 " + (-width) + "px"});
   }else{
    $(this).parent("div").css({ backgroundPosition: "0 0px"});
...
img.src = found.value;
	...
	<style type="text/css">
        ...
        .memJoinForm .input{border:none;border-bottom:1px solid #ddd;}
        .grayLineBtn1{border:none;background:none;border-bottom:1px solid #ddd;}
        .basic_btn_area{margin:0;width:100%;}
        .basic_btn_area .join_bt{margin:0;background:#766dc1;}
        .form1 label input{display:none;}

		#fruitimage{width:100%;}
		#fruitimage img{width:100%;}
		.form1{width:48.444vw;margin:0px auto;}
		.form1>div{display:flex;justify-content:space-around;font-size:4.444vw;margin:2.222vw auto;width:100%;}
		
        .ff_se{margin:0px auto;text-align:center;}
        
		.ff_se>input[type="radio"]{display: none;}
		.ff_se>label{display:inline-block;}
		
		.ff_se>.r1{background:url(./img/he1.png)no-repeat;background-size:cover;width:62px;height:82px;margin-right:3.333vw;}
		.ff_se>.r2{background:url(./img/he2.png)no-repeat;background-size:cover;width:62px;height:82px;}
		
		.ff_se>input[type="radio"]:checked+.r1{background:url(./img/fe1.png)no-repeat;background-size:cover;}
		.ff_se>input[type="radio"]:checked+.r2{background:url(./img/fe2.png)no-repeat;background-size:cover;}
