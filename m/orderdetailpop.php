...
if ($type=="cancel" || ($type=="okescrow" && $rescode=="C" && strlen($ordercode)>0)) { //매매보호 주문거절시
	...
	if($row=mysql_fetch_object($result)) {
		...
		|| ($_data->ordercancel=="2" && $row->deli_gbn=="N" && getDeligbn("N",true)) // 발송준비 전 :: tblorderproduct에 deli_gbn이 "N"만 있는지 확인한다.
...
<div class="orderdetailwrap">
	...
	<div style="text-align:center; margin:15px 0px;">
		<A HREF="javascript:window.close();" class="basic_button grayLineBtn btnPaddingRadius">닫기</A>
		<?
		if($print!="OK") {
			$deliAfterCancelYN = TRUE;
